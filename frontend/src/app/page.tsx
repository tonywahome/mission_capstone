            ...
            {!isAuthenticated ? (
              ...
              <div className="animate-fade-up-delay-2 mt-10 flex flex-col sm:flex-row gap-3 justify-center">
                {user?.role === "steward" ? (
                  ...
                  <Link href="/landowner/pending-scans" className="rounded-xl bg-white text-terra-800 px-7 py-3.5 text-sm font-bold hover:bg-terra-50 shadow-xl hover:-translate-y-0.5 transition-all">
                    Verification queue
                  </Link>
                )}
              </div>
        ...
        <div className="relative max-w-6xl mx-auto px-4">
          ...
          <div className="grid md:grid-cols-3 gap-6">
            <Pillar
              ...
              body="Sentinel-2 optical and Sentinel-1 radar composites fused with NASA GEDI spaceborne LiDAR feed an XGBoost biomass model locally calibrated on Rwandan A/R sites — achieving ≥74% RMSE reduction over global products."
            />
            <Pillar
              step="02"
            ...
            <Pillar
              ...
              body="Every scan and verification decision is logged with its uncertainty range and integrity score — traceable from satellite pixel to field plot, with full-precision data restricted to Research-Administrator review."
            />
          </div>
        </div>
      </section>
        ...
        <div className="max-w-6xl mx-auto px-4">
          ...
          <div className="grid md:grid-cols-3 gap-6">
            <RoleCard
              icon="🌳"
              title="Land Stewards"
              body="Register project boundaries, upload field-plot data, and track biomass estimates with built-in uncertainty ranges."
              cta="Request steward access"
              href="/signup"
              accent
            />
            <RoleCard
            ...
            <RoleCard
              icon="🛡️"
              title="Research Administrators"
              body="Full-precision access to unrounded GPS coordinates and model internals, per the capstone's ethical-safeguard review."
              cta="Researcher sign-in"
              href="/login"
            />
          </div>
        </div>
      ...
      <section className="py-14 bg-[var(--color-surface-muted)] border-t border-[var(--color-border)]">
        <div className="max-w-6xl mx-auto px-4 text-center">
          ...
          <div className="flex flex-wrap justify-center gap-x-10 gap-y-3">
            {["Sentinel-1 · ESA", "Sentinel-2 · ESA", "GEDI · NASA", "GHG Protocol", "Verra VCS", "ICVCM Core Carbon Principles", "Rwanda NDC 2030"].map((item) => (
